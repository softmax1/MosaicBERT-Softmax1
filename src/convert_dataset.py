# Downloaded on 2023-08-20 from https://github.com/mosaicml/examples/blob/main/examples/benchmarks/bert/src/convert_dataset.py

# Copyright 2022 MosaicML Examples authors
# SPDX-License-Identifier: Apache-2.0

"""Streaming dataset conversion scripts for C4."""
import os
import platform
import warnings
...
def parse_args() -> Namespace:
    ...
    parser.add_argument('--data_subset', type=str, default='en', help='E.g. "all" or "en"')
    parser.add_argument('--splits', nargs='+', default=['train_tiny', 'val_tiny'])
    parser.add_argument('--out_root', type=str, required=True)
    parser.add_argument('--compression', type=str, default=None)

    group = parser.add_mutually_exclusive_group(required=False)
    group.add_argument('--concat_tokens', type=int, help='Convert text to tokens and concatenate up to this many tokens')
    ...
    if os.path.isdir(parsed.out_root) and len(set(os.listdir(parsed.out_root)).intersection(set(parsed.splits))) > 0:
        raise ValueError(f'--out_root={parsed.out_root} contains {os.listdir(parsed.out_root)} which cannot overlap with the requested splits {parsed.splits}.')
...
c4constants.splits['train_medium'] = DataSplitConstants(
    hf_split='train',
    folder_split='train_medium',
    raw_samples=61358080,
    truncated_samples=61358080
)
c4constants.splits['train_tiny'] = DataSplitConstants(
    hf_split='train',
    folder_split='train_tiny',
    raw_samples=4096,
    truncated_samples=4096
)
...
c4constants.splits['val_tiny'] = DataSplitConstants(
    hf_split='validation',
    folder_split='val_tiny',
    raw_samples=4096,
    truncated_samples=4096
)

CONSTS = {'c4': c4constants}
    ...
    def __iter__(self) -> Iterable[Dict[str, bytes]]:
        ...
        for sample in self.hf_dataset:
            ...
            while len(buffer) >= self.max_length:
                ...
                yield {
                    # convert to bytes to store in MDS binary format
                    'tokens': asarray(concat_sample).tobytes()
                }
...
def _est_progress_denominator(total_samples: int,
                              chars_per_sample: int,
                              chars_per_token: int,
                              mode: ConcatMode,
                              max_length: int):
...
def main(args: Namespace) -> None:
    ...
    try:
        ...
        raise ValueError(f'Constants for dataset "{args.dataset}" not found. Currently only "c4" is supported.')
    ...
    for split_name in args.splits:
        ...
        with MDSWriter(columns=columns, out=os.path.join(args.out_root, folder_split), compression=args.compression) as out:
